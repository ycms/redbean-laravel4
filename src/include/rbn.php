        ...
        public function countOwn($type)
        {
            ...
            if ($this->getID()) {
                ...
                if (!is_numeric($firstKey) || $firstKey === null) {
                    ...
                    $bindings = array_merge(array($this->getID()), $this->withParams);
                    $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount($type, array(), " {$joinSql} $myFieldLink = ? " . $this->withSql,
                        $bindings);
                }
            ...
            foreach ($bean as $property => $value) {
                ...
                if ($value instanceof OODBBean) {
                    ...
                    if (strpos($property, 'own') === 0) {
                        ...
                        list($sharedAdditions, $sharedTrashcan, $sharedresidue) = $this->processGroups($originals, $value, $sharedAdditions, $sharedTrashcan,
                            $sharedresidue);
                        unset($bean->$property);
                    }
                }
